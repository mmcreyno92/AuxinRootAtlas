...
pdf(file = "30min_IAA_upset_transcripts.pdf", width = 55, height = 20, fonts = "sans") #Create Upset Plot image as a .PDF
upset(fullUpDnDE, nsets = 8, nintersects = 20, empty.intersections = "on", order.by = "freq", point.size = 8, line.size = 3.5, keep.order = T,
      mainbar.y.label = "Shared Transcripts", sets.x.label = "Total DE Transcripts",
      mb.ratio = c(0.55,0.45), text.scale = c(8,8,4.5,5,5,7), number.angles = 30)
dev.off()

svg("30min_IAA_upset_transcripts.svg", width = 55, height = 20) #Create Upset Plot image as a .SVG
upset(fullUpDnDE, nsets = 8, nintersects = 20, empty.intersections = "on", order.by = "freq", point.size = 8, line.size = 3.5, keep.order = T,
      mainbar.y.label = "Shared Transcripts", sets.x.label = "Total DE Transcripts",
      mb.ratio = c(0.55,0.45), text.scale = c(8,8,4.5,5,5,7), number.angles = 30)
dev.off()
...
fullUpDnDE %>%
  filter(.$`EZ auxin t30 up` == 1 & .$`C auxin t30 up` == 1 & .$`S auxin t30 up` ==1) %>% #add column ids here
  rowwise() %>%
